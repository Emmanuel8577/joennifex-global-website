...
export default function OrderForm() {
  ...
  const toggleItem = (product) => {
    const exists = selectedItems.find((i) => i.name === product.name);
    
    if (exists) {
      setSelectedItems((prev) => prev.filter((i) => i.name !== product.name));
    } else {
      // Clean item price for event value tracking
      const cleanPrice = parseFloat(String(product.price).replace(/[^0-9.-]+/g, "")) || 0;
      
      // Fire AddToCart event for middle-of-funnel retargeting audiences
  ...
  const handleSubmit = async (e) => {
    ...
    const productNames = selectedItems.map((i) => i.name).join(", ");

    formData.set("products", productNames);
    ...
    try {
      ...
      if (response.ok) {
        // Fully Optimized Meta Lead Payload with Product Details & Advanced Match Parameters
        pixel.trackEvent(
          "Lead",
          {
            content_name: productNames || "Solar Order Inquiry",
            content_category: "Solar Equipment",
            contents: selectedItems.map((item) => ({
              id: item.name,
              quantity: 1,
              item_price: parseFloat(String(item.price).replace(/[^0-9.-]+/g, "")) || 0,
            })),
            value: total,
            currency: "NGN",
          ...
          {
            em: emailVal.trim().toLowerCase(),
            ph: phoneVal.trim().replace(/[^0-9+]/g, ""),
            fn: nameVal.trim().split(" ")[0] || "",
          }
        );
        ...
        {submitted ? (
          ...
          >
            ...
            <button 
              ...
              className="mt-6 text-yellow-600 font-bold underline"
            >
              Place another order
            </button>
          </motion.div>
          ...
          <motion.form
            ...
            action="https://formspree.io/f/mbdbzrda" 
            method="POST"
          ...
          >
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              <button 
                ...
                className="w-full p-4 border rounded-xl bg-white flex justify-between items-center hover:border-yellow-600 transition"
              >
            ...
            {['name', 'phone', 'email', 'address'].map((field) => (
              <input 
                ...
                className="w-full p-4 border rounded-xl focus:ring-2 focus:ring-yellow-500 outline-none" 
                required 
              />
            ))}
            ...
            <button 
              type="submit" 
              className="w-full p-4 bg-gray-900 text-white rounded-xl font-bold hover:bg-yellow-600 transition-all shadow-lg hover:-translate-y-1"
            >
              Confirm Order
            </button>
          </motion.form>
